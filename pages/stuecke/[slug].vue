<template>
    <div>
        <PageHero :title="pageTitle" :breadcrumbs="[
            { label: 'HOME', to: '/' },
        ...
        <div class="mx-auto max-w-7xl p-4 sm:p-8 lg:max-w-7xl">
            ...
            <div v-else-if="loadingState === 'error'">
                Leider konnte die Veranstaltung nicht gefunden werden. Bitte versuche es später erneut.
            ...
            <div v-else-if="event">
                ...
                <section class="my-8 flex flex-row gap-x-8">
                    ...
                    <div :class="{ 'basis-2/3': event.picture }">
                        ...
                        <p class="my-8">
                            EMPFOHLEN AB {{ event.recommendedAge || '4' }} JAHREN | Eintritt: 16 € - 10 € ermässigt |
                            Gruppen ab 12 Personen 1 Euro erm. pro Platz | Eine THEATERDECK Produktion in Kooperation
                            mit der Theater Jugend Hamburg e.V. und dem
                            Amt für Familie | Die Rechte liegen beim Verlag für Kindertheater Weitendorf GmbH
                        </p>
                    </div>
                </section>
                ...
                <section class="bg-white">
                    <div ref="pretixWidgetContainer"></div>
                    <noscript>
...
const eventURI = computed(() => `${config.public.pretixBaseUrl}/td/${slug.value}/`)
...
onUnmounted(() => {
    const pretixOverlay = document.querySelector('div.pretix-widget-alert-holder')
    if (pretixOverlay) {
...
// Some helper Texts
const descMissing = 'Hier sollte eigentlich die Beschreibung des Events sein, aber es ist leider ein Fehler aufgetreten.'
